...
const results=fs.existsSync(statusFile)?JSON.parse(fs.readFileSync(statusFile,'utf8')).filter(r=>r.status==='complete'&&fs.existsSync(path.join(output,r.id+'.mp3'))):[];
...
for(const bank of banks.filter(b=>b.installable!==false)){
 if(results.some(r=>r.id===bank.id))continue;
 ...
 const japanese=!bank.languageCodes?.includes('en')&&bank.languageCodes?.includes('ja');if(japanese)score.lyricsLanguage='ja';
 score.parts[0].notes=score.parts[0].notes.slice(0,3).map((n,i)=>({...n,lyric:(bank.testLyrics||(japanese?['あ','い','う']:['ah','hello','woo']))[i],duration:480,syllabic:'single'}));
