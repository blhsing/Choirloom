...
export class PreviewPlayer {ctx:AudioContext|null=null;nodes:AudioNode[]=[];started=0;offset=0;duration=0;playing=false;
 ...
 play(score:Score,offset=0,solo:string[]=[]){this.stop();this.ctx??=new AudioContext();void this.ctx.resume();const ctx=this.ctx;this.started=ctx.currentTime;this.offset=offset;this.duration=secondsAt(score,endTick(score));this.playing=true;
