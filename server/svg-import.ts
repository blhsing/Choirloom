...
export async function rasterizeScoreSvg(bytes:Buffer):Promise<Buffer>{
 ...
 const scale=Math.min(4,3200/Math.max(width,height),Math.sqrt(8_000_000/(width*height)));const w=Math.max(1,Math.floor(width*scale)),h=Math.max(1,Math.floor(height*scale));if(!validView)svg['@_viewBox']=`0 0 ${width} ${height}`;svg['@_width']=String(w);svg['@_height']=String(h);
