...
test('structured imports save immediately as a revision without a review gate',async()=>{
 const a=await request('/auth/signup','POST',{name:'Direct import',email:'direct-import@example.com',password:'long password 123'}),p=await request('/projects','POST',{demo:true},a.cookie,a.data.csrf);
 const boundary='choirloom-direct-import',xml='<score-partwise><part-list><score-part id="P1"><part-name>Voice</part-name></score-part></part-list><part id="P1"><measure number="1"><attributes><divisions>1</divisions><time><beats>3</beats><beat-type>4</beat-type></time></attributes><note><pitch><step>C</step><octave>4</octave></pitch><duration>3</duration></note><barline location="right"><repeat direction="backward"/></barline></measure></part></score-partwise>',body=Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="file"; filename="score.musicxml"\r\nContent-Type: application/xml\r\n\r\n${xml}\r\n--${boundary}--\r\n`);
 const imported:any=await new Promise((resolve,reject)=>{const req=httpRequest(root+`/projects/${p.data.id}/import`,{method:'POST',headers:{origin:process.env.APP_ORIGIN!,cookie:a.cookie!,'x-csrf-token':a.data.csrf,'content-type':`multipart/form-data; boundary=${boundary}`,'content-length':body.length}},res=>{let text='';res.on('data',d=>text+=d);res.on('end',()=>resolve({status:res.statusCode,data:JSON.parse(text)}));});req.on('error',reject);req.end(body);});
 assert.equal(imported.status,200);assert.equal(imported.data.version,2);assert.equal(imported.data.reviewRequired,undefined);const saved=await request(`/projects/${p.data.id}`,'GET',undefined,a.cookie);assert.equal(saved.data.version,2);assert.equal(saved.data.score.beats,3);assert.equal(saved.data.score.barlines[0].repeat,'backward');
});
...
test('interrupted import cleanup preserves committed shared voices',async()=>{
 ...
 run("INSERT INTO voice_library(bank_id,status) VALUES('committed-voice','installed')");recoverVoiceImports();assert.equal(existsSync(work),false);assert.equal(existsSync(asset),true);assert.equal(one('SELECT status FROM voice_imports WHERE id=?',id).status,'completed');rmSync(asset);run('DELETE FROM voice_imports WHERE id=?',id);run("DELETE FROM voice_library WHERE bank_id='committed-voice'");
});
