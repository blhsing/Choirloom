...
export class PreviewPlayer {prepareGeneration=0;samples:LoadedSample[]=[];sampleInstrument:PreviewInstrument|null=null;
 ...
 play(score:Score,offset=0,solo:string[]=[],instrument:PreviewInstrument='piano'){score=expandRepeats(score);this.stop();this.ctx??=new AudioContext();void this.ctx.resume();const ctx=this.ctx;this.started=ctx.currentTime+.05;this.offset=offset;this.duration=secondsAt(score,endTick(score));this.playing=true;
 ...
 if(this.sampleInstrument===instrument&&this.samples.length){
