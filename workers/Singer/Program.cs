...
foreach(var part in input.GetProperty("parts").EnumerateArray()){
 ...
 var englishVowels=new HashSet<string>(vowelSet);
 ...
 for(int ni=0;ni<notes.Length;ni++){if(notes[ni].GetProperty("syllabic").GetString()!="begin")continue;int end=ni;while(end+1<notes.Length&&notes[end].GetProperty("syllabic").GetString()!="end")end++;var word=Clean(string.Concat(notes.Skip(ni).Take(end-ni+1).Select(n=>n.GetProperty("lyric").GetString())));if(!EnglishPronunciation.TryGet(dictionary,word,out var full))continue;var vowelIndexes=full.Select((p,i)=>(p,i)).Where(x=>englishVowels.Contains(x.p.Split('/').Last())).Select(x=>x.i).ToArray();int cursor=0;for(int j=ni;j<=end;j++){int cut=j==end?full.Length:Math.Clamp((j-ni+1<vowelIndexes.Length?Math.Max(vowelIndexes[j-ni]+1,vowelIndexes[j-ni+1]-1):(j-ni+1)*full.Length/(end-ni+1)),cursor+1,full.Length);resolved[notes[j].GetProperty("id").GetString()!]=full[cursor..cut];cursor=cut;if(cursor>=full.Length)break;}ni=end;}
...
if(opts.ContainsKey("--validate-lyrics"))return;
