...
if(-not $AssetsOnly){
 ...
 try { Get-ChildItem -LiteralPath $stage -Recurse -File | ForEach-Object { $relative=[IO.Path]::GetRelativePath($stage,$_.FullName).Replace('\','/');$include=$relative -match '^(app/|config/|dist/|runtime/|singer/|node_modules/|package(-lock)?\.json$|web\.config$|entry\.cjs$)';if($CodeOnly){$include=$relative -match '^(app/|config/|dist/|web.config|entry.cjs|singer/(?!System\.|Microsoft\.|core|clr|host|ms|vcruntime|onnx|libmp3))'};if($IncludeDependencies -and $relative -match '^(node_modules/(ws/|pdfjs-dist/|@napi-rs/)|package(-lock)?\.json$)'){$include=$true};if($ApiOnly){$include=$relative -match '^(app/|config/|entry.cjs|web.config)'};if($WorkerOnly){$include=$relative -match '^singer/(?!System\.|Microsoft\.|core|clr|host|ms|vcruntime|onnx|libmp3)'};if($include){[IO.Compression.ZipFileExtensions]::CreateEntryFromFile($zip,$_.FullName,$relative,[IO.Compression.CompressionLevel]::Fastest) | Out-Null} } } finally {$zip.Dispose();$stream.Dispose()}
